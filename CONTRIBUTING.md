...
4. Checkout the appropriate branch for the version you're working on (for example, `8.0` for version 8.0 documentation) and pull the latest changes from upstream:

    ```shell
        git checkout 8.0
        git pull upstream 8.0
    ```

    !!! note "Which branch to use"
    
        Use the branch that matches the version of the documentation you're editing. For version 8.0 documentation, use the `8.0` branch. If the branch doesn't exist locally, Git will create a tracking branch automatically.
