...
When setting up Percona repositories, you can choose between two paths: Percona Distribution for MySQL (the full stack documented here) or Percona Server for MySQL alone. Both use the same core database engine but are designed for different operational scopes.

To read about the standalone server product, see the [Percona Server for MySQL 8.0 :octicons-link-external-16:](https://docs.percona.com/percona-server/8.0/) documentation.
...
| Feature | Percona Distribution for MySQL (`pdps-8.0`) — *this product* | Percona Server for MySQL (`ps-80`) |
| --- | --- | --- |
...
* Percona Distribution for MySQL (this product): [Install Percona Distribution for MySQL 8.0](installing.md)

* Percona Server for MySQL (standalone): [Install Percona Server for MySQL 8.0 :octicons-link-external-16:](https://docs.percona.com/percona-server/8.0/installation.html)
