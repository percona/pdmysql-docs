...
2. Select a product, for example, Percona Distribution for MySQL.
3. In `Product Group`, select which product, for example, Percona Server for MySQL.
4. In `Version`, select the distribution version, for example, `percona-distribution-mysql-ps-8.0.41`.
